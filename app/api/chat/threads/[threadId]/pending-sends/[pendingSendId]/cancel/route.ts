...
export async function POST(
  _request: Request,
  { params }: { params: { threadId: string; pendingSendId: string } }
) {
  const baseUrl = internalApiBaseUrl();
  const res = await fetch(
    `${baseUrl}/chat/threads/${encodeURIComponent(params.threadId)}/pending-sends/${encodeURIComponent(params.pendingSendId)}/cancel`,
    {
      method: "POST",
